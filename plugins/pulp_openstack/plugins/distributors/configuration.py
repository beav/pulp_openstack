...
def validate_config(config):
    ...
    if server_url:
        ...
        if not parsed.scheme:
            errors.append(PulpCodedValidationException(error_code=error_codes.GLA1001,
                                                       field=constants.CONFIG_KEY_REDIRECT_URL,
                                                       url=server_url))
        if not parsed.netloc:
            errors.append(PulpCodedValidationException(error_code=error_codes.GLA1002,
                                                       field=constants.CONFIG_KEY_REDIRECT_URL,
                                                       url=server_url))
        if not parsed.path:
            errors.append(PulpCodedValidationException(error_code=error_codes.GLA1003,
                                                       field=constants.CONFIG_KEY_REDIRECT_URL,
                                                       url=server_url))
    ...
    if protected:
        ...
        if protected_parsed is None:
            errors.append(PulpCodedValidationException(error_code=error_codes.GLA1004,
...
def get_redirect_url(config, repo):
    ...
    if redirect_url:
        ...
        redirect_url = 'https://%s/pulp/openstack/%s/' % (server_name, repo.id)
