...
class PublishImagesStep(UnitPublishStep):
    ...
    def process_unit(self, unit):
        ...
        target_base = self.get_web_directory()
    ...
    def finalize(self):
        """
        Close & finalize with the metadata context
        """
        # create working dir. If one already exists, do nothing
        try:
            os.mkdir(self.get_web_directory())
        except OSError, e:
            if e.errno == errno.EEXIST:
                pass
        repo_metadata_filename = os.path.join(self.get_web_directory(), '.image-metadata.xml')
